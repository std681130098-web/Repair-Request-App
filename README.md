...
- **แนบรูปภาพจุดที่ต้องซ่อมได้หลายรูป** (ดู/ลบได้ระหว่างที่ใบยังแก้ไขได้)
...
| **RepairImage** | รูปภาพประกอบใบแจ้งซ่อม (แนบได้หลายรูป) |
...
|-------|----------|----------|
| ผู้ดูแลระบบ (superuser) | `admin` | `1234` |
| ช่างซ่อม | `tech1`, `tech2` | `1234` |
| ผู้แจ้งซ่อม | `user1`, `user2` | `1234` |

หน้าผู้ดูแลระบบของ Django อยู่ที่ 👉 **http://127.0.0.1:8000/admin/**

### 🛠️ เข้าหน้า /admin ไม่ได้ / ลืมรหัสผ่าน

ถ้าได้ไฟล์ `Backup.zip` มาแล้วเข้าหน้าแอดมินไม่ได้ ให้รันคำสั่งนี้เพื่อ
สร้างหรือรีเซ็ตบัญชีแอดมินกลับเป็น `admin` / `1234`:

```bash
python manage.py create_admin          # รีเซ็ตเฉพาะบัญชี admin
python manage.py create_admin --all    # รีเซ็ตบัญชีทดสอบทั้งหมดด้วย
python manage.py create_admin --password mypass123   # กำหนดรหัสผ่านเอง
```

> ⚠️ **หมายเหตุด้านความปลอดภัย** — รหัสผ่าน `1234` เป็นค่าสำหรับเดโม/ส่งงานเท่านั้น
> ถ้านำระบบไปใช้งานจริง ต้องเปลี่ยนรหัสผ่านทันทีหลัง login ครั้งแรก
> และไม่ควรเก็บรหัสผ่านจริงไว้ในซอร์สโค้ดหรือใน Git
